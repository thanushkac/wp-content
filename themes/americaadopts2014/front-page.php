...
<?php
$args = array( 'posts_per_page' => 5, 'order'=> 'DEC', 'orderby' => 'post_date', 'category' => 96, );
...
if ( $query->have_posts() ) {
    ...
    while ( $query->have_posts() ) {
		echo '<div>';
        $query->the_post();
		echo '' .the_post_thumbnail('thumbnail', array('class' => 'homepostthumb pull-left'));
...
<?php
$args = array( 'posts_per_page' => 4, 'order'=> 'DEC', 'orderby' => 'post_date', 'category' => 5, );
$postslist = get_posts( $args );
foreach ( $postslist as $post ) :
  setup_postdata( $post ); ?> 
	<div>
