...
from
wp_posts p
join wp_postmeta pm1 on p.ID = pm1.post_id and pm1.meta_key = 'isSearchable'
left join wp_postmeta pm10 on p.ID = pm10.post_id and pm10.meta_key = 'isPremium'
";

if (!empty($formEthnicity) && $formEthnicity != '0') {
  $query .= "
left join wp_postmeta pm2 on p.ID = pm2.post_id and pm2.meta_key = 'partner1Ethnicity'
left join wp_postmeta pm3 on p.ID = pm3.post_id and pm3.meta_key = 'partner2Ethnicity'";
}
if (!empty($formReligion) && $formReligion != '0') {
  $query .= "
left join wp_postmeta pm4 on p.ID = pm4.post_id and pm4.meta_key = 'partner1Religion'
left join wp_postmeta pm5 on p.ID = pm5.post_id and pm5.meta_key = 'partner2Religion'";
}
if (!empty($formRegion) && $formRegion != '0') {
  $query .= "
left join wp_postmeta pm6 on p.ID = pm6.post_id and pm6.meta_key = 'region'";
}
